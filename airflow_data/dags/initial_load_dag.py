...
# === Sequential Batch Processing Configuration ===
# Processes all companies in batches of 2, sequentially
# Batch 1: AAPL, MSFT → Batch 2: GOOGL, AMZN → ... → Batch 6: TM, C
...
# Get all companies from companies.yaml
all_companies = get_companies_list()

# Configuration: Process N companies at a time
BATCH_SIZE = 2
...
# Create dummy operators for batch boundaries
batch_starts = []
batch_ends = []
...
for batch_idx, batch in enumerate(batches):
    ...
    # Create start marker for this batch
    batch_start = DummyOperator(
        task_id=f'batch_{batch_num}_start',
        dag=dag
    )
    batch_starts.append(batch_start)
    
    # Create tasks for companies in this batch (run in parallel within batch)
    batch_tasks = []
    for company in batch:
        task = PythonOperator(
            task_id=f'process_{company["ticker"]}',
    ...
    # Create end marker for this batch
    batch_end = DummyOperator(
        task_id=f'batch_{batch_num}_end',
        dag=dag
    )
    batch_ends.append(batch_end)
    
    # Set dependencies within this batch
    batch_start >> batch_tasks >> batch_end

end = DummyOperator(task_id='end', dag=dag)

# === Set Sequential Dependencies Between Batches ===
# Batch 1 → Batch 2 → Batch 3 → ... → End

# Start → load_companies → First batch
start >> load_companies_task >> batch_starts[0]

# Chain batches sequentially: batch_1_end → batch_2_start → batch_2_end → batch_3_start ...
for i in range(len(batches) - 1):
    batch_ends[i] >> batch_starts[i+1]
